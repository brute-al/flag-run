...
// A handful of buildings secretly carry a powerup (OVERCHARGE / SPLASH /
...
console.log("\n=== Powerups hidden in buildings ===");
{
  ...
  game.chooseVehicle("tank");
  const arena = game.arena;
  const VALID_TYPES = ["overcharge", "splash", "laser", "armor"];
  ...
  check("OVERCHARGE quadruples damage", game._weaponModifiers().damageMult === 4);

  game.activePowerup = { type: "splash", timeLeft: 5 };
  const splashMod = game._weaponModifiers();
  check("SPLASH keeps a normal bullet radius (that was the whole problem with BIG SHOT)", splashMod.radiusMult === 1);
  check("SPLASH still hits a little harder on direct impact", splashMod.damageMult > 1);
  check("SPLASH carries a splash radius", splashMod.splashRadius > 0);
...
console.log("\n=== Powerup seeding re-randomizes each round ===");
{
  ...
  check(
    ...
    seededRound2.every((o) => ["overcharge", "splash", "laser", "armor"].includes(o.powerupType))
  );
  check(
...
// --- 23. SPLASH powerup: area damage around the impact, normal bullet radius
// Replaced BIG SHOT (see game.js's POWERUP_STATS comment): a fatter bullet
// radius made shots clip buildings the player wasn't aiming at. SPLASH keeps
// `radiusMult: 1` -- the bullet's own hit test is completely ordinary -- and
// instead damages nearby destructible buildings/turrets once it lands (see
// Game._applySplash). Direct placement, same style as the piercing tests
// above: two destructible buildings are pulled close enough together that
// splash range is guaranteed to reach the second one, regardless of where
// the real map actually put them.
console.log("\n=== SPLASH powerup: area damage on impact ===");
{
  const input = makeInput();
  const game = new Game(input);
  game.chooseVehicle("tank");
  const destructibles = game.arena.obstacles.filter((o) => o.destructible && !o.destroyed);
  const target = destructibles[0];
  const neighbor = destructibles[1];

  if (!target || !neighbor) {
    check("found two destructible buildings to test splash against", false);
  } else {
    neighbor.x = target.x + 30;
    neighbor.y = target.y;
    const targetHealth = target.health;
    const neighborHealth = neighbor.health;

    console.log("[a splash round direct-hits the target for full damage]");
    const bullet = new Bullet(target.x, target.y, 0, 0, 20, true);
    bullet.splashRadius = 55;
    bullet.splashDamageMult = 0.5;
    game.bullets = [bullet];
    game.update(dt);
    check("the directly-hit building took the shot's full damage", target.health === targetHealth - 20);

    console.log("[the same shot also chips a nearby building it never touched directly]");
    check("the neighboring building lost health too", neighbor.health < neighborHealth);
    check(
      "but only the reduced splash share, not a second full hit",
      neighbor.health === neighborHealth - 20 * 0.5
    );

    console.log("[without splashRadius set, a shot never reaches past whatever it hit]");
    neighbor.x = target.x + 30;
    neighbor.y = target.y;
    const neighborHealthBefore = neighbor.health;
    const plainBullet = new Bullet(target.x, target.y, 0, 0, 15, true);
    game.bullets = [plainBullet];
    game.update(dt);
    check("a non-splash shot leaves the neighbor untouched", neighbor.health === neighborHealthBefore);
  }
}
