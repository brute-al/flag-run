...
const POWERUP_STATS = {
  // Bumped from 2x to 4x per playtesting -- 2x didn't read as different
  // enough shot-to-shot to feel worth grabbing over the other three.
  overcharge: { damageMult: 4, radiusMult: 1, piercing: false, damageTakenMult: 1 },
  // Replaced BIG SHOT: a fatter `radiusMult` sounded fun but actually made
  // the shot's own hitbox wider, so it clipped buildings well off the actual
  // aim line -- and since it wasn't piercing, that shot died right there
  // instead of reaching whatever it was aimed at (see the building-collision
  // check below, `o.radius + bullet.radius`). SPLASH keeps `radiusMult: 1` --
  // a completely normal bullet in flight -- and instead deals area damage
  // around wherever it actually lands (`splashRadius` world units,
  // `splashDamageMult` of the direct-hit damage), consumed in the
  // building/turret collision handling further down. So the "big impact"
  // feeling now comes from a satisfying explosion on hit, not a wider miss
  // window on the way there.
  splash: { damageMult: 1.2, radiusMult: 1, piercing: false, damageTakenMult: 1, splashRadius: 55, splashDamageMult: 0.5 },
...
export class Game {
  ...
  _weaponModifiers() {
    ...
    if (!stats) return { damageMult: 1, radiusMult: 1, piercing: false, damageTakenMult: 1, splashRadius: 0, splashDamageMult: 0 };
    return stats;
  }

  // SPLASH's actual effect: called once at the exact point/moment a bullet
  // stops on a building or turret (see the collision-handling block below),
  // this damages every *other* destructible building and live turret within
  // `bullet.splashRadius` of the impact point, at `splashDamageMult` of
  // whatever this shot's own damage was. `excludeObstacle`/`excludeTurret`
  // is whichever object the bullet directly hit -- it already took full
  // damage via the normal path just above the call site, so it's skipped
  // here to avoid double-dipping the one thing that got a clean hit.
  // Deliberately scoped to buildings/turrets only, not vehicles -- the
  // complaint this replaced was specifically about unwanted building
  // collisions, so that's what it fixes; splashing player/AI health as well
  // would be a separate balance call nobody's asked for yet.
  _applySplash(bullet, x, y, excludeObstacle, excludeTurret) {
    if (!bullet.splashRadius) return;
    const splashDamage = bullet.damage * bullet.splashDamageMult;
    for (const o of this.arena.obstacles) {
      if (o === excludeObstacle || o.destroyed || !o.destructible) continue;
      if (Math.hypot(o.x - x, o.y - y) > o.radius + bullet.splashRadius) continue;
      o.health -= splashDamage;
      this.events.push("buildingHit");
      if (o.health <= 0) this._destroyBuilding(o);
    }
    for (const t of this.turrets) {
      if (t === excludeTurret || t.destroyed) continue;
      if (Math.hypot(t.x - x, t.y - y) > t.radius + bullet.splashRadius) continue;
      t.takeDamage(splashDamage);
      this.events.push("turretHit");
      if (t.destroyed) {
        this.events.push("turretDestroyed");
        this.particles.fieryExplosion(t.x, t.y, 1.3);
        this.camera.shake(8, 0.3);
      }
    }
    // A visible burst at the impact point itself, distinct from the small
    // per-hit spark every bullet already gets, so a splash impact reads as
    // meaningfully bigger than a normal one even against a target with no
    // destructible neighbors nearby.
    this.particles.fieryExplosion(x, y, 0.7);
  }
  ...
  update(dt, canvasW = 1600, canvasH = 900) {
    ...
    // Hovercraft-style movement (see Vehicle.update's `omni` handling): all
    // three player-driveable vehicles use it now -- the jeep's old car-style
    // steering started feeling like a holdover from before twin-stick aim
    // once the tank switched over, and the heli's own nose-relative flight
    // started feeling the same way once the ground vehicles went hovercraft
    // (per user feedback: "rotating and going is starting to feel weird").
    // For the heli, `omni` only changes how the stick *moves* it -- its nose
    // keeps independently tracking `aimAngle` exactly as before (see
    // Vehicle.update's `isAerial`-first heading branch), so twin-stick aim
    // is untouched; only translation changed. The duel-mode AI opponent's
    // tank (driven separately, below, via aiDriver.getVector()) keeps the
    // old turn-to-face driving aiDriver.js's pursuit steering assumes, and
    // it never drives a heli, so that case is untouched here.
    this.vehicle.update(dt, {
      ...rawInput,
      aimAngle: aim.angle,
      omni: this.vehicle.type === "tank" || this.vehicle.type === "jeep" || this.vehicle.type === "heli",
    });
    ...
    if (this.vehicle.weapon && aim.active && aim.angle !== null && !useSecondary && this.vehicle.weapon.cooldown <= 0) {
      ...
      // A powerup pickup (see below) can juice this shot: OVERCHARGE
      // quadruples damage, SPLASH deals area damage around wherever it lands
      // (bullet itself stays normal width -- see splashRadius/
      // splashDamageMult below), LASER pierces through whatever it hits
      // instead of stopping on the first thing.
      ...
      bullet.splashRadius = mod.splashRadius || 0;
      bullet.splashDamageMult = mod.splashDamageMult || 0;
      this.bullets.push(bullet);
    ...
    if (this.vehicle.weapon2 && aim.active && aim.angle !== null && useSecondary && this.vehicle.weapon2.cooldown <= 0) {
      ...
      missile.splashRadius = mod.splashRadius || 0;
      missile.splashDamageMult = mod.splashDamageMult || 0;
        ...
        for (const o of this.arena.obstacles) {
          ...
          if (Math.hypot(bullet.x - o.x, bullet.y - o.y) < o.radius + bullet.radius) {
            ...
            if (o.destructible) {
              ...
              // SPLASH: this shot also damages whatever destructible cover
              // or turret is standing nearby (see _applySplash) -- every
              // other powerup leaves splashRadius at 0, so this is a no-op
              // for them.
              this._applySplash(bullet, bullet.x, bullet.y, o, null);
            }
      ...
      if (bullet.friendly) {
        for (const turret of this.turrets) {
          ...
          if (Math.hypot(bullet.x - turret.x, bullet.y - turret.y) < bullet.radius + turret.radius) {
            ...
            // SPLASH: same area-damage pass as the building-hit case above,
            // just triggered from a turret hit instead -- a splash round can
            // take out a turret and singe the building next to it, or vice
            // versa, in one shot.
            this._applySplash(bullet, bullet.x, bullet.y, null, turret);
            if (turret.destroyed) {
              this.events.push("turretDestroyed");
    ...
    if (this.state !== "respawning") {
      drawables.push({
        ...
        draw: () => {
          ...
          // Only the player's own vehicle carries a powerup, so only it gets
          // the active-buff visual (shield bubble / energy aura) -- the
          // duel-mode AI opponent below never picks one up.
          drawVehicle(ctx, s.x, s.y, this.vehicle, healthFrac, VEHICLE_LIFT, this.activePowerup && this.activePowerup.type);
        },
      });
    }
    if (this.duel && this.aiVehicle && this.aiRespawnTimer <= 0) {
